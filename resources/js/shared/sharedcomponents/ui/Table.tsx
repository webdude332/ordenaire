...
export const TableContainerOne = ({
    ...
    <div
        className={`overflow-hidden rounded-t-xl border border-gray-200 bg-white shadow-sm ${className}`}
    >
        {children}
    </div>
);
...
export const TableHeader = ({
    ...
    <thead
        className={`border-b border-borderColor bg-gray-50 text-xs text-gray-500 ${className}`}
    >
        <tr>{children}</tr>
    </thead>
);
...
>(({ children, className = '', style }, ref) => (
    <tr ref={ref} style={style} className={`hover:bg-gray-50 ${className}`}>
        {children}
    </tr>
));
