...
export default function EditAppModal({
    ...
    const [pricingModel, setPricingModel] = useState('recurring_monthly');
    ...
    useEffect(() => {
        if (app) {
            setAppName(app.name);
            setCategory(app.category.toLowerCase());
            setBasePrice(
                app.pricing === 'Free'
                    ? '0.000'
                    : app.pricing.replace(' KWD', ''),
            );
            setPricingModel(
                app.pricing === 'Free' ? 'free' : 'recurring_monthly',
            );
            setAppClientId(`app_id_89234_${app.name.toLowerCase()}`);
        }
    }, [app]);
                    ...
                    <div className="rounded-xl border border-gray-200 p-5">
                        ...
                        <div className="mb-4 grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <Input
                                    placeholder="app_id_89234_talabat"
                                    value={appClientId}
                                    onChange={(e) =>
