...
const SearchableDropdown = ({
    ...
    return (
        <div className="relative space-y-2">
            ...
            >
                {label}
                {required && <span className="ml-1 text-[#8CDD05]">*</span>}
            </label>

            <div
                onClick={handleToggle}
                className={`relative flex w-full cursor-pointer items-center justify-between rounded-lg border px-3 py-2.5 text-sm shadow-xs transition-all ${
                    disabled
            ...
            >
                <div className="flex w-full items-center gap-2">
                    ...
                    <SearchIcon
                        className={`h-4 w-4 ${isOpen ? 'text-[#8CDD05]' : 'text-gray-400'}`}
                    />
