...
// Fixed: converts Sun=0...Sat=6 → Mon=0...Sun=6 for Mo-Su grid
const getFirstDayOfMonth = (year: number, month: number) => {
...
const Calendar = ({ year, month, selectedRange, onDateClick }: any) => {
    ...
    // Next month fill to complete 42-cell grid
                ...
                {days.map((d, idx) => {
                    ...
                    return (
                        <div key={idx} className="relative">
                            <button
                                onClick={() => onDateClick(d.date)}
                                className={`relative flex h-9 w-9 items-center justify-center rounded-full text-sm ${!d.isCurrentMonth ? 'text-gray-300' : 'text-gray-700'} ${isStart || isEnd ? 'z-10 bg-[#79B800] font-medium text-white' : ''} ${inRange ? 'bg-green-50' : ''} ${!isStart && !isEnd && !inRange ? 'hover:bg-gray-100' : ''} `}
                            >
                                {d.day}
                                {showToday && !isStart && !isEnd && (
...
export default function CustomDateRangePicker({ onClose, onApply }: Props) {
    ...
    const handlePresetClick = (preset: (typeof PRESETS)[0]) => {
        ...
        // Navigate the left calendar to the month that contains range.from
        setCurrentDate(
