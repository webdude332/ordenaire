...
const SingleDatePicker = ({ onApply, onClose }: Props) => {
    const today = startOfDay(new Date());

    const [isOpen, setIsOpen] = useState(false);
    const [selectedDate, setSelectedDate] = useState<Date>(today);
    const [committedDate, setCommittedDate] = useState<Date>(today); // what's shown in the trigger
    ...
    // Re-sync internal selection when opening
    const handleOpen = () => {
    ...
    const handleCancel = () => {
        setSelectedDate(committedDate); // revert
        setIsOpen(false);
        onClose?.();
    };
    ...
    return (
        <div
            className="relative inline-block w-full max-w-[240px]"
            ref={containerRef}
        >
            {/* TRIGGER */}
            <div
            ...
            {/* DROPDOWN */}
            {isOpen && (
                <div className="absolute left-0 z-50 mt-2 w-[310px] rounded-[20px] border border-gray-100 bg-white p-4 shadow-2xl">
                        ...
                        {days.map((item, idx) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative mx-auto flex h-9 w-9 items-center justify-center rounded-full text-[13px] transition-all outline-none ${isFuture ? 'cursor-not-allowed text-gray-200' : 'cursor-pointer'} ${!item.current && !isFuture ? 'text-gray-300 hover:bg-gray-50' : ''} ${item.current && !isSelected && !isFuture ? 'text-gray-700 hover:bg-gray-50' : ''} ${isSelected ? 'bg-[#79B800] font-bold !text-white shadow-sm' : ''} `}
                                >
