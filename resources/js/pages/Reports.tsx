...
//         (typeof icon === 'object' && icon !== null && !icon.src)
//     ) {
...
//                                                         }
//                                                     >
//                                                         <RunBtn className="h-4 w-4 text-iconColor" />
//                                                         Run Now
//                                                     </ActionButton>
//                                                     <ActionButton
//                                                         onClick={() =>
...
//                                                         }
//                                                     >
//                                                         <PencilBtn className="h-4 w-4 text-iconColor" />
//                                                     </ActionButton>
//                                                     <ActionButton
//                                                         onClick={() =>
//                                                             openDeleteModal(
...
//                                                         }
//                                                     >
//                                                         <DelBtn className="h-4 w-4 text-iconColor" />
//                                                     </ActionButton>
//                                                 </div>
//                                             </td>
//                                         </tr>
...
//                             <div className="flex items-center gap-2">
//                                 <button className="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-50 text-sm font-medium text-gray-900 ring-1 ring-gray-200 ring-inset">
//                                     1
//                                 </button>
...
//                                                 ) : (
//                                                     <ActionButton>
//                                                         <DownloadBtn className="h-4 w-4 text-iconColor" />
//                                                         Download
//                                                     </ActionButton>
//                                                 )}
//                                             </td>
...
// Mock Data
const upcomingReports = [
    { title: 'Monthly Summary', tenant: 'Acme Corp', frequency: 'Monthly', nextRun: '03 Sept 2025 7:30 AM', status: 'Scheduled' },
    { title: 'Weekly Update', tenant: 'Acme Corp', frequency: 'Weekly', nextRun: '04 Sept 2025 01:15 PM', status: 'Completed' },
    { title: 'Weekly Review', tenant: 'Stark Ind.', frequency: 'Weekly', nextRun: '05 Sept 2025 09:45 AM', status: 'In Progress' },
    { title: 'Daily Report', tenant: 'Pizza Hut', frequency: 'Daily', nextRun: '05 Sept 2025 09:45 AM', status: 'Pending' },
];

const historyReports = [
    { title: 'Monthly Summary', tenant: 'Acme Corp', sentOn: '03 Sept 2025 7:30 AM', recipients: 'abc@gmail.com', status: 'Sent' },
    { title: 'Weekly Update', tenant: 'Acme Corp', sentOn: '04 Sept 2025 01:15 PM', recipients: 'xyz@example.com', status: 'Sent' },
    { title: 'Quarterly Review', tenant: 'Stark Ind.', sentOn: '05 Sept 2025 09:45 AM', recipients: 'info@company.com', status: 'Sent' },
    { title: 'Annual Report', tenant: 'Pizza Hut', sentOn: '05 Sept 2025 09:45 AM', recipients: 'contact@business...', status: 'Failed' },
];
...
const Label = ({ children }: { children: React.ReactNode }) => (
    <label className="mb-1.5 block text-sm font-bold text-gray-900">{children}</label>
);

const Input = ({ placeholder, icon, iconClassName = 'text-gray-400' }: { placeholder: string; icon?: any; iconClassName?: string }) => (
    <div className="relative">
        {icon && (
            <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
...
const Select = ({ placeholder, options, icon = DownArrow }: { placeholder: string; options?: string[]; icon?: any }) => (
    <div className="relative">
        <select className="w-full appearance-none rounded-lg border border-gray-300 bg-white py-2.5 pr-10 pl-3 text-sm text-gray-900 shadow-sm focus:border-[#84cc16] focus:ring-1 focus:ring-[#84cc16] focus:outline-none">
            <option>{placeholder}</option>
...
const Checkbox = ({ label, defaultChecked, icon = checkIcon }: { label: string; defaultChecked?: boolean; icon?: any }) => {
    return (
        <label className="group flex cursor-pointer items-center gap-2 text-sm text-gray-700">
...
export default function ReportsPage() {
    ...
    const openErrorDetailsModal = () => setIsErrorDetailsModalOpen(true);
    const handleRunConfirm = () => {
        setIsRunModalOpen(false);
        setTimeout(() => setIsSuccessModalOpen(true), 500);
    };
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-[#F9FAFB] font-montserrat">
            ...
            <div className="relative flex min-w-0 flex-1 flex-col overflow-hidden">
                <header className="sticky top-0 z-10 bg-white px-8 pt-6">
                    <nav className="mb-3 flex items-center text-xs font-medium text-gray-500">
                        <span><RenderIcon icon={AnalyticsIcon} className="h-4 w-4 text-gray-400" /></span>
                        <span className="mx-2 text-gray-300"><RenderIcon icon={ArrowRight} className="h-4 w-4 text-[#B5B0BA]" /></span>
                        <span className="font-semibold text-[#82798B]">Dashboard</span>
                        <span className="mx-2 text-gray-300"><RenderIcon icon={ArrowRight} className="h-4 w-4 text-[#B5B0BA]" /></span>
                        <span className="rounded-sm bg-[#F9F7FA] px-3 py-1.5 font-semibold text-[#4F4955]">Reports page</span>
                    </nav>
                    <h1 className="mb-6 text-2xl font-bold tracking-tight text-gray-900">Reports page</h1>
                    <div className="flex space-x-8 border-b border-gray-200 text-sm font-medium text-gray-500">
                        <span className="border-b-2 border-[#79B800] pb-3">
                            <p className="font-semibold text-[#578500]">Dashboard</p>
                        </span>
                    </div>
                </header>

                <main className="custom-scrollbar flex-1 overflow-y-auto bg-white p-8">
                    <Link href="/dashboard" className="mb-6 inline-flex items-center gap-2 rounded-lg border border-[#CFCBD2] bg-white px-4 py-2 text-sm font-medium text-[#4F4955] shadow-sm transition-colors hover:bg-gray-50">
                        <RenderIcon icon={ArrowBack} className="h-5 w-5 text-[#B5B0BA]" />
                        <span className="font-[600] text-[#4F4955]">Back to Dashboard</span>
                    </Link>
                    ...
                    <div className="mb-8 w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                        ...
                        <div className="grid grid-cols-1 gap-6 px-6 pt-6 md:grid-cols-2">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <CustomDropdown label="Report Template" options={templateOptions} value={reportTemplate} onChange={setReportTemplate} placeholder="templates—Sales, Usage, Financial" />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mb-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                        <div className="flex flex-col items-center justify-between gap-4 border-b border-gray-200 p-6 md:flex-row">
                            ...
                            <div className="flex w-full gap-3 md:w-auto">
                                <div className="relative flex-1 md:w-64">
                                    ...
                                    <input type="text" placeholder="Search" className="w-full rounded-lg border border-gray-300 py-2 pr-3 pl-10 text-sm shadow-sm focus:border-[#84cc16] focus:ring-1 focus:ring-[#84cc16] focus:outline-none" />
                                </div>
                                <button className="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-gray-50">
                                    <RenderIcon icon={calenderPng} /> Date-range
                                </button>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-[#F9F7FA]">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-left text-sm font-semibold tracking-wider text-gray-500">
                                            <div className="flex cursor-pointer items-center gap-1">Next Run <RenderIcon icon={SelectorIcon} className="h-4 w-4 text-[#B5B0BA]" /></div>
                                        </th>
                                        <th className="px-6 py-4 text-left text-sm font-semibold tracking-wider text-gray-500">
                                ...
                                <tbody className="divide-y divide-gray-200 bg-white">
                                    {upcomingReports.map((item, idx) => (
                                        <tr key={idx} className="transition-colors hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-2">
                                                    <ActionButton onClick={() => openRunModal(item.title)}><RunBtn className="h-4 w-4 text-iconColor" />Run Now</ActionButton>
                                                    <ActionButton onClick={() => openEditModal(item.title)}><PencilBtn className="h-4 w-4 text-iconColor" /></ActionButton>
                                                    <ActionButton onClick={() => openDeleteModal(item.title)}><DelBtn className="h-4 w-4 text-iconColor" /></ActionButton>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                        ...
                        <div className="flex items-center justify-between border-t border-gray-200 px-6 py-4">
                            ...
                            <div className="flex items-center gap-2">
                                <button className="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-50 text-sm font-medium text-gray-900 ring-1 ring-gray-200 ring-inset">1</button>
                                <button className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 hover:text-gray-700">2</button>
                            </div>
                            <button className="flex h-8 w-8 items-center justify-center rounded-lg border border-gray-300 bg-white shadow-sm transition-colors hover:bg-gray-50">
                                    ...
                                    {historyReports.map((item, idx) => (
                                        <tr key={idx} className="transition-colors hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                                                {item.status === 'Failed' ? (
                                                    <div className="flex items-center gap-2">
                                                        <ActionButton onClick={openErrorDetailsModal}>View Error</ActionButton>
            ...
            {/* Modals */}
            <RunReportModal isOpen={isRunModalOpen} onClose={() => setIsRunModalOpen(false)} onConfirm={handleRunConfirm} />
            <EditReportModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onSave={handleEditSave} />
            <SimpleErrorModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onRetry={handleDelete} />
            <ErrorDetailsModal isOpen={isErrorDetailsModalOpen} onClose={() => setIsErrorDetailsModalOpen(false)} onRetry={() => {}} />
