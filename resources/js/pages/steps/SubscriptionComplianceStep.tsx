...
const SubscriptionStep = ({ data, update, onNext, onBack, isEditMode = false }: StepProps) => {
    const [isUploadModalOpen, setIsUploadModalOpen] = useState(false);
    const [selectedDocTitle, setSelectedDocTitle] = useState('');
    const [isAddDocModalOpen, setIsAddDocModalOpen] = useState(false);

    const handleOpenUpload = (docName: string) => { setSelectedDocTitle(docName); setIsUploadModalOpen(true); };
    const handleCloseUpload = () => { setIsUploadModalOpen(false); setSelectedDocTitle(''); };
    const handleConfirmUpload = () => { console.log(`Uploaded ${selectedDocTitle}`); setIsUploadModalOpen(false); };
    const handleOpenAddDoc = () => { setIsAddDocModalOpen(true); };
    const handleCloseAddDoc = () => { setIsAddDocModalOpen(false); };
    ...
    const RadioGroup = ({ options, value, onChange }: { options: string[]; value: string; onChange: (val: string) => void; }) => (
        <div className="flex items-center gap-6">{options.map((option) => (<label key={option} className="flex cursor-pointer items-center gap-2"><div className={`flex h-5 w-5 items-center justify-center rounded-full border ${value === option ? 'border-[#8CDD05]' : 'border-gray-300'} bg-white`}>{value === option && (<div className="h-2.5 w-2.5 rounded-full bg-[#8CDD05]" />)}<input type="radio" className="hidden" checked={value === option} onChange={() => onChange(option)} /></div><span className="text-sm text-gray-700">{option}</span></label>))}</div>
    );
            ...
            <div className="space-y-6 pt-6">
                ...
                <div className="grid grid-cols-12 gap-10 border-t border-b border-[#E8E6EA] pt-8 pb-10">
                    ...
                    <div className="col-span-9 space-y-6 rounded-xl border border-[#E8E6EA] bg-white p-6 shadow-xs">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-1.5"><label className="text-sm font-medium text-gray-700">Start Date<span className="text-[#8CDD05]">*</span></label><input type="text" value={data.startDate} onChange={(e) => update('startDate', e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-500 outline-none focus:border-[#8CDD05] focus:ring-1 focus:ring-[#8CDD05]" /></div>
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            ...
                            <div className="space-y-3"><div className="space-y-3"><label className="text-sm font-medium text-gray-700">Auto-renew</label><div className="pt-2"><RadioGroup options={['Enabled', 'Disabled']} value={data.autoRenew} onChange={(val) => update('autoRenew', val)} /></div></div></div>
                        </div>
                        <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="flex flex-col gap-6"><div><label className="text-sm font-medium text-gray-700">Discount Type (Optional)</label><div className="mt-2"><RadioGroup options={['Percentage', 'Amount']} value="Percentage" onChange={() => {}} /></div></div></div>
                                <div className="flex flex-col justify-end"><div className="flex gap-2"><input type="text" placeholder="Enter Value" className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm outline-none focus:border-[#8CDD05] focus:ring-1 focus:ring-[#8CDD05]" /><button className="whitespace-nowrap rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">Apply</button></div></div>
                            </div>
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {documents.map((doc, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="px-6 py-4">{doc.isUploaded ? (<span className="inline-flex items-center rounded-lg border border-[#ABEFC6] bg-[#ECFDF3] px-2.5 py-0.5 text-xs font-medium text-[#027A48]"><div className="mr-1.5 h-1.5 w-1.5 rounded-sm bg-[#12B76A]"></div>{doc.status}</span>) : (<span className="inline-flex items-center rounded-lg border border-borderColor bg-gray-50 px-2.5 py-0.5 text-xs font-medium text-gray-600"><div className="mr-1.5 h-1.5 w-1.5 rounded-full bg-gray-500"></div>{doc.status}</span>)}</td>
                                        <td className="px-6 py-4 text-gray-900">{doc.expiry}</td>
                                        <td className="px-6 py-4 text-right">{doc.isUploaded ? (<button className="inline-flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50"><DelIcon className="h-4 w-4 text-iconColor" />Delete</button>) : (<button onClick={() => handleOpenUpload(doc.name)} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50"><Upload className="h-3.5 w-3.5" />Upload</button>)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        <div className="flex justify-end border-t border-gray-200 p-4"><button onClick={handleOpenAddDoc} className="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"><Plus className="h-4 w-4" />Add other document</button></div>
                    </div>
                </div>
                ...
                <UploadDocumentModal isOpen={isUploadModalOpen} onClose={handleCloseUpload} onUpload={handleConfirmUpload} title={selectedDocTitle} />
                <AddDocModal isOpen={isAddDocModalOpen} onClose={handleCloseAddDoc} onConfirm={() => { console.log('Document Added'); handleCloseAddDoc(); }} />
            </div>
        </div>
    );
};
