...
const AddUser = () => {
    ...
    const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if ((val === '' || /^[0-9]+$/.test(val)) && val.length <= 8) {
            ...
            if (val.length > 0 && val.length < 8) {
                setErrors((prev) => ({ ...prev, phoneNumber: 'Phone number must be at least 8 digits.' }));
            } else {
    ...
    const handleDeleteDoc = () => {
        if (selectedDocIndex !== null) {
            setDocuments((prev) => prev.filter((_, i) => i !== selectedDocIndex));
        ...
        <div className="flex min-h-screen">
            ...
            <main className="flex flex-1 flex-col">
                <div className="sticky top-0 z-10 bg-white">
                    <TopBar title="Add New User" icon={DashBoardIcon} breadcrumbs={breadcrumbs} />
                </div>
                    ...
                    <form className="space-y-8 pb-12" onSubmit={(e) => e.preventDefault()}>
                        ...
                        <div className="grid grid-cols-1 gap-8 border-t border-[#E8E6EA] py-6 lg:grid-cols-3">
                            <div className="lg:col-span-1">
                                ...
                                <p className="mt-1 text-sm text-gray-500">Add User photo and personal details.</p>
                            </div>
                            ...
                            <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm lg:col-span-2">
                                ...
                                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                    <div>
                                        ...
                                        <Input placeholder="eg., Noah Pierre" value={fullName} onChange={handleFullNameChange} error={errors.fullName} />
                                    </div>
                                    <div>
                                        <Label className="mb-2 text-sm font-medium">Email Address <span className="text-primary">*</span></Label>
                                        <Input placeholder="eg., Noah@ordermark.com" icon={MailIcon} value={email} onChange={handleEmailChange} error={errors.email} />
                                    </div>
                                    <div>
                                        <Label className="mb-2 text-sm font-medium">Phone Number <span className="text-primary">*</span></Label>
                                        <div className={`flex rounded-lg border shadow-xs transition-colors focus-within:ring-1 ${errors.phoneNumber ? 'border-red-500 focus-within:ring-red-500' : 'border-gray-300 focus-within:border-[#84cc16] focus-within:ring-[#84cc16]'}`}>
                                            ...
                                            <Input placeholder="1825462385" value={phoneNumber} onChange={handlePhoneChange} className="!rounded-l-none !rounded-r-lg !border-0 !shadow-none focus:!ring-0" />
                                        </div>
                                        {errors.phoneNumber && <p className="mt-1.5 text-xs font-medium text-red-600">{errors.phoneNumber}</p>}
                                    </div>
                                    <div>
                                        <CustomDropdown label="Primary Role" options={templateOptions} value={reportTemplate} onChange={setReportTemplate} placeholder="Customer relationship Manager" labelClassName="mb-2 text-sm font-medium" />
                                    </div>
                                </div>
                                <div className="mt-6">
                        ...
                        <div className="flex items-center justify-end gap-4 border-t border-borderColor pt-4">
                            ...
                            <Button type="submit">Send invite & Add User</Button>
                        </div>
                    </form>
                </div>
            </main>
